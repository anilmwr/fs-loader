...
class FileSystemLoader:
    def __init__(self, mount_directory: str, sub_directory_depth: int,
                 file_count_per_directory: int,
        ...
        if os.path.isdir(mount_directory):
            ...
            if os.access(mount_directory, os.W_OK | os.X_OK):
                ...
                raise ValueError(f'Mount directory exists but not writable: \
                                 {mount_directory}')
        else:
            raise ValueError(f'Mount directory doesn\'t exist: \
                             {mount_directory}')
        ...
        for parameter in [sub_directory_depth, file_count_per_directory,
                          sub_directory_count, block_size, min_block_count,
                          max_block_count]:
            if not isinstance(parameter, int) or parameter <= 0:
                raise ValueError(f'Positive integer expected, received: \
                                 {repr(parameter)}')
    ...
    def load_fs(self) -> None:
        ...
        for directory_ids in itertools.product(
                range(1, self.sub_directory_count + 1),
                repeat=self.sub_directory_depth):
            ...
            for i in range(1, len(directory_ids) + 1):
                sub_dir_name = os.path.join(
                    current_directory_path, self.directory_prefix +
                    '_'.join(map(str, directory_ids[:i])))
                current_directory_path = os.path.join(
                    current_directory_path, sub_dir_name)

            try:
                os.makedirs(current_directory_path)
            except IOError as e:
                logging.error(f'Unable to create directory \
                              {current_directory_path}: {str(e)}')
                return

            # repeat for number of files to be created in sub directory
            for file_index in range(1, self.file_count_per_directory + 1):
                current_file_path = os.path.join(current_directory_path,
                                                 str(file_index) + '.txt')

                # get random integer in the range min_block_count
                # and max_block_count inclusive
                file_block_count = random.randint(self.min_block_count,
                ...
                try:
                    ...
                    logging.error(f'Unable to create file '
                                  f'{current_file_path}: {str(e)}')
                    return
    ...
    def display_progress(self) -> None:
        ...
        print(f'\rFiles created: {self.current_file_count:12d} Completed: '
              f'{progress_percent:3d}%', end='')

        while (progress_percent < 100):
            print(f'\rFiles created: {self.current_file_count:12d} Completed: '
                  f'{progress_percent:3d}%', end='')
            current_progress_percent = \
                int((self.current_file_count / file_count_tobe_created) * 100)
